...
{
    // Test FEN parsing
    Position pos = position_from_fen(START_FEN);
    ...
    for (int r = 0; r < 8; ++r)
    {
        for (int c = 0; c < 8; ++c)
        {
            PieceCode p = pos.board[get_sq(r,c)];
            TEST_ASSERT(p == BOARD[r][c]);
        }
    }

    TEST_ASSERT(pos.black_to_move == false);

    const uchar CASTLE_ALL = CASTLE_WK | CASTLE_WQ | CASTLE_BK | CASTLE_BQ;
    TEST_ASSERT(pos.castle_flags == CASTLE_ALL);
    TEST_ASSERT(pos.ep_target == NO_EP_TARGET);
    TEST_ASSERT(pos.halfmove == 0);
    TEST_ASSERT(pos.fullmove == 1);

    // Black to move position after 1. e4
    const char FEN1[] = 
        "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1";
   
    pos = position_from_fen(FEN1);
    ...
    for (int r = 0; r < 8; ++r)
    {
        for (int c = 0; c < 8; ++c)
        {
            TEST_ASSERT(pos.board[get_sq(r,c)] == BOARD[r][c]);
        }
    }
    TEST_ASSERT(pos.black_to_move == true);
    TEST_ASSERT(pos.castle_flags == CASTLE_ALL);
    TEST_ASSERT(pos.ep_target == sq_from_coord("e3"));
    TEST_ASSERT(pos.halfmove == 0);
    TEST_ASSERT(pos.fullmove == 1);
}
